...
function player_update_visuals(pl)
	...
	default.player_anim[name] = 0 -- Animation will be set further below immediately
...
function player_step(dtime)
	for _, pl in pairs(minetest.get_connected_players()) do
		...
		if pl:get_hp() == 0 then
			if default.player_anim[name] ~= ANIM_LAY then
				pl:set_animation({x=anim.lay_START, y=anim.lay_END}, animation_speed_mod, animation_blend)
				default.player_anim[name] = ANIM_LAY
			end
		elseif walking and controls.LMB then
			if default.player_anim[name] ~= ANIM_WALK_MINE then
				pl:set_animation({x=anim.walk_mine_START, y=anim.walk_mine_END}, animation_speed_mod, animation_blend)
				default.player_anim[name] = ANIM_WALK_MINE
			end
		elseif walking then
			if default.player_anim[name] ~= ANIM_WALK then
				pl:set_animation({x=anim.walk_START, y=anim.walk_END}, animation_speed_mod, animation_blend)
				default.player_anim[name] = ANIM_WALK
			end
		elseif controls.LMB then
			if default.player_anim[name] ~= ANIM_MINE then
				pl:set_animation({x=anim.mine_START, y=anim.mine_END}, animation_speed_mod, animation_blend)
				default.player_anim[name] = ANIM_MINE
			end
		elseif default.player_anim[name] ~= ANIM_STAND then
			pl:set_animation({x=0, y=79}, animation_speed_mod, animation_blend)
			default.player_anim[name] = ANIM_STAND
		end
	end
end
